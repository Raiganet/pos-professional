...
export async function GET() {
  ...
  try {
    ...
    const start7 = new Date(startToday); start7.setDate(start7.getDate() - 6);   // 7 hari incl. hari ini
    ...
    // Map nama produk (untuk top products)
    const nameMap: Record<string, string> = {};
    products.forEach((p) => { nameMap[p.id] = p.nama; });

    // Grafik 7 hari (kronologis: 6 hari lalu → hari ini)
    ...
    // Aktivitas terbaru (gabungkan transaksi + mutasi stok)
    const recent = [
    ...
    return NextResponse.json(defaults, { status: 200 }); // tetap 200 + data kosong → UI aman
  }
}import { NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';
