...
console.log('\n🚀 POS PROFESSIONAL - SETUP TAHAP 5 (LAPORAN)\n');
console.log('===============================================\n');

// ==========================================
// 1. API ROUTES - REPORTING
// ==========================================
console.log('🔌 Membuat API Routes Laporan...');

createFile(path.join(ROOT_DIR, 'app/api/reports/sales/route.ts'), `
...
createFile(path.join(ROOT_DIR, 'app/api/reports/products/route.ts'), `
import { NextRequest, NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';
...
// ==========================================
// 2. HALAMAN LAPORAN UTAMA
// ==========================================
console.log('\n📊 Membuat halaman Laporan...');

createFile(path.join(ROOT_DIR, 'app/(dashboard)/laporan/page.tsx'), `
'use client';
...
if (fs.existsSync(sidebarPath)) {
  ...
  if (!content.includes("label: 'Laporan'")) {
     // Pastikan menu Laporan ada
     const insertPoint = "{ label: 'Pengaturan', icon: Settings, href: '/pengaturan' },";
     content = content.replace(insertPoint, "{ label: 'Laporan', icon: BarChart3, href: '/laporan' },\n  " + insertPoint);
     fs.writeFileSync(sidebarPath, content);
     console.log('✅ Updated: Sidebar menu Laporan');
  }
}
...
// ==========================================
console.log('\n===============================================');
console.log('✅ TAHAP 5 SELESAI!');
console.log('===============================================\n');
console.log('Modul Laporan siap digunakan:\n');
console.log('   📈 Sales Report: Lihat omzet & profit harian/bulanan');
console.log('   🏆 Top Products: Identifikasi barang paling laku');
console.log('   📥 Export Excel: Download data untuk akuntansi');
console.log('   🖨️  Print Ready: Cetak laporan langsung dari browser\n');
console.log('Silakan buka halaman /laporan di browser Anda.');